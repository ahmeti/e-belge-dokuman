		...
		<html>
			...
			<body style="margin-left=0.6in; margin-right=0.6in; margin-top=0.79in; margin-bottom=0.79in">
						...
						<tbody>
							<tr valign="top">
								...
								<td width="40%" align="middle" valign="middle">
									<xsl:choose>
										<xsl:when test="$param_logo">
											<img alt="Logo_Custom" style="max-width:250px;max-height:150px;" src="data:image/jpg;base64,{$param_logo}"/>
										</xsl:when>
										<xsl:when test="$var_logo">
											<img alt="Logo_Custom" style="max-width:250px;max-height:150px;" src="data:image/jpg;base64,{$var_logo}"/>
										</xsl:when>
										<xsl:otherwise>
											<!-- img alt="Logo" style="max-width:150px;max-height:90px;" src="data:image/jpg;base64,/9j/..."/> -->
										</xsl:otherwise>
									</xsl:choose>
								</td>
							</tr>
						...
						<tr align="left">
							<td id="notesTableTd">
								...
								<xsl:for-each select="//n1:Invoice/cac:AdditionalDocumentReference">
									...
									<xsl:if test="./cbc:DocumentTypeCode = 'EREPSENDT' and //n1:Invoice/cac:AdditionalDocumentReference/cbc:ID='ELEKTRONIK'">
										<b>&#160;&#160;&#160;&#160;&#160;</b>
								...
								<xsl:for-each select="//n1:Invoice/cbc:Note">
									<xsl:if test="substring-before(.,':')='NOT'">
										<b>&#160;&#160;&#160;&#160;&#160;</b>
				...
				<xsl:if test="//n1:Invoice/cac:AdditionalDocumentReference/cbc:DocumentTypeCode = 'EUPRWEBURI'">
					<table class="notesTable"  align="left" height="50" cellpadding="2" cellspacing="2" style="margin-top:10px;margin-bottom:10px;width:704px;text-align:left; color:black;">
						<tr>
							<td id="notesTableTd">

								<div style="width:100%;text-align:left;float:left;">

									<div style="width:100%;text-align:center;margin:6px;">
										<span style="font-weight:bold;">
									...
									<table border="1" id="lineTable" align="left" width="100%">
										<tbody>
											<tr>
			...
			<td id="lineTableTd" align="right">
				<span>
					...
					<xsl:if test="./cac:AllowanceCharge/cbc:MultiplierFactorNumeric">
						<xsl:text> %</xsl:text>
						<xsl:variable name="mfn" select="./cac:AllowanceCharge/cbc:MultiplierFactorNumeric"/>
						<xsl:value-of select="format-number(mfn*100, '###.##0,00', 'european')"/>
					</xsl:if>
				</span>
			</td>
			<td id="lineTableTd" align="right">
				<span>
					...
					<xsl:if test="./cac:AllowanceCharge">
						...
						<xsl:variable name="amnt" select="./cac:AllowanceCharge/cbc:Amount"/>
						<xsl:value-of select="format-number(amnt, '###.##0,00', 'european')"/>
					</xsl:if>
